...
/* Load the data */
const promises = [
  d3.json('static/global/data/ne_50m_admin_0_countries.json'),
  d3.json('static/points/data/ne_10m_populated_places_simple.geojson'),
];
...
Promise.all(promises).then((data) => {
    ...
    const padding = 30;
    width =
      window.innerWidth <= 360 ? 300 : window.innerWidth <= 960 ? 600 : 1200;
    const getHeight = () => {
...
const render = ({projection,path,cityPath,graticule,graticulePath,countries,citiesData,}) => {
  ...
  context.beginPath(), graticulePath(graticule), (context.strokeStyle = '#023333'), context.stroke();
...
const transition = (p, renderArgs) => {
  ...
  // Mutates projection
  const { projection, path, cityPath, graticule, graticulePath, countries, citiesData, } = renderArgs;

  d3.transition().duration(850).tween('rotate', function () {
...
const drawGraph = (projection, path, countriesData, citiesData) => {
  ...
  const countries = topojson.feature(countriesData, { type: 'GeometryCollection', geometries: countriesData.objects['ne_50m_admin_0_countries'].geometries,});

  const renderArgs = { projection, path, cityPath, graticule, graticulePath, countries, citiesData, };
  ...
  d3.geoZoom().projection(projection).onMove(() => render(renderArgs))(context.canvas);
